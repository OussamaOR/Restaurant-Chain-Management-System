...
float SalesVec::max_sale(){
    ...
    for(int i = 0;i<Salesvec.size();i++){
        if(Salesvec[i].getDailySaleAmount() > current) current = Salesvec[i].getDailySaleAmount();
    }
    return current;
}
...
float SalesVec::total_onmonth(int year,int month){
    float total = 0.f;
    if(getDailySaleIndex(1,month,year) >= Salesvec.size()) return 0;
...
float SalesVec::total_onyear(int year){
    float total = 0.f;
    if(getDailySaleIndex(1,1,year) >= Salesvec.size()) return 0;
...
float SalesVec::total_onperiod(Date start_date,Date end_date){
    float total = 0.f;
    if(getDailySaleIndex(getDailySaleIndex(start_date.getDay(), start_date.getMonth(), start_date.getYear())) >= Salesvec.size()) return 0;
