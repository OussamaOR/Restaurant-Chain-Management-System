...
float SalesVec::total_onperiod(Date start_date,Date end_date){
    float total = 0.f;
    for(int i = getDailySaleIndex(start_date.getDay(),start_date.getMonth(),start_date.getYear());i<=getDailySaleIndex(end_date.getDay(),end_date.getMonth(),end_date.getYear());i++){
        total += Salesvec[i].getDailySaleAmount();
    }
    return total;
}